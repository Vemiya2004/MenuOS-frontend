...
let activeBannerFilter = null; // { type, data } — null means no filter active

async function loadBanners() {
    try {
        const res = await fetch(`${API_BASE_URL}/admin/api/banners/public`);
        ...
        track.innerHTML = banners.map((b, i) => {
            ...
            return `
            <div class="banner-slide" onclick="applyBannerFilter(${i})" data-index="${i}">
                ${hasImage
                    ? `<img src="${b.image_url}" alt="${b.title}" onerror="this.style.display='none'">`
                    : ''}
        ...
        // Store banners for filter use
        window._bannerData = banners;

        // Auto-scroll every 4s if more than 1
        if (banners.length > 1) {
...
function goToBanner(index) {
    ...
    const len = (window._bannerData || []).length;
    if (len > 1) {
        bannerAutoTimer = setInterval(() => {
            bannerCurrentIndex = (bannerCurrentIndex + 1) % len;
            updateBannerPosition();
        }, 4000);
    }
}
...
function updateBannerPosition() {
    ...
    if (track) track.style.transform = `translateX(-${bannerCurrentIndex * 100}%)`;
    dots.forEach((d, i) => d.classList.toggle('active', i === bannerCurrentIndex));
}

async function applyBannerFilter(index) {
    const banner = (window._bannerData || [])[index];
    if (!banner) return;

    let linked = banner.linked_data;
    if (typeof linked === 'string') { try { linked = JSON.parse(linked); } catch(e){ linked = []; } }

    activeBannerFilter = { type: banner.link_type, data: linked };

    // Show loading
    showLoading(true);

    if (banner.link_type === 'category') {
        // Load items from all selected categories
        let allItems = [];
        for (const cat of linked) {
            const url = `${API_BASE_URL}/api/menu?category=${encodeURIComponent(cat)}`;
            try {
...
async function loadData() {
    ...
    renderMenuItems();
    showLoading(false);
    // Load banners after menu (non-blocking)
    loadBanners();
}
